 ...
 * assignment #5.
 * 
 * @author Peter Jensen and ---Jonas Jacobs---
...
public class WordleClone {
	...
	public static void main(String[] args) {
		...
		while (guessCount <= 6) {
			...
			/*
			 * TODO: Write the logic that validates the user's guess. (One simple 'if'
			 * statement and a few controlled statements.)
			 */
			...
			/*
			 * TODO: Write the logic that checks for a win. (A few statements total,
			 * including one 'if' statement.)
			 */
	...
	/**
	 * Creates a random number given the number of items inside of the five.txt
	 * file. loops through the random number given changing the word variable to the
	 * next 5 letter word inside the file. then returns the randomized five letter
	 * word.
	 * 
	 * @param filename "five.txt"
	 * @param randInt  a randomized int between [0..the number of items inside
	 *                 five.txt]
	 * @param word     the variable that holds the randomized string
	 * 
	 * @return returns the randomized word and if one is not found returns "aaron"
	 */
	public static String pickRandomWord(String filename) {
		int wordCount = countWords(filename);
		int randInt = (int) ((Math.random() * wordCount) + 1);
		String word = "";
	...
	/**
	 * Given a word and a filename, this method determines if the word is in the
	 * file. True is returned if the word is in the file, and false is returned
	 * otherwise.
	 * 
	 * Note that the word should not have any whitespace in it, or it won't match
	 * anything scanned from the file. (No special check is done here for that
	 * case.) The reason is that this function uses the .next() function from the
	 * Scanner class, and this strips away whitespace.
	 * 
	...
	/**
	 * Given a user's guess and a Wordle answer, this method 'scores' the guess. It
	 * returns something that looks like a copy of the guess: Guess characters
	 * appear to be replaced with a '-' if they don't exist in the answer. They
	 * remain unchanged if they exist in the answer but are in the wrong spot. They
	 * are changed to uppercase if they're in the correct spot.
	 * 
	 * This function requires five character strings of letters.
	 * 
	 * For example: answer: miter guess: timid score: tIm--
	 * 
	 * Note that each letter in the guess or answer is only scored once. Thus, even
	 * though there were multiple i's in the guess, only one was scored.
	 * 
...
//		 (Just 'continue' in that case.)
		...
		for (int a = 0; a < answer.length(); a++) {
			for (int g = 0; g < guess.length(); g++) {
				char wordelAnswer = answer.charAt(a);
				char playerGuess = guess.charAt(g);
				if (wordelAnswer == '-' || playerGuess == '-') {
					continue;
				}
				if (wordelAnswer == playerGuess) {
					score = replaceLetter(score, g, playerGuess);
				}
			}
		}
		...
		for (int i = 0; i < guess.length(); i++) {
			...
			if (a == c) {
				score = replaceLetter(score, i, Character.toUpperCase(c));
			}
		}
