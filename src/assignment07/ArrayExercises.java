...
public class ArrayExercises
{
    ...
    public static double[] computeAreas (double[] widths, double[] heights)
    {
    	...
    	for(int i = 0; i < widths.length; i++) {
    		if(widths == null || heights == null) {
    			...
    			double temp = widths[i] * heights[i];
    			areas[areaIndex] = temp;
    			areaIndex++;
    		}
    	}
        return areas; // Stub
...

    {
    	...
    	for(int i = 0; i < pixels.length; i++) {
    		if(pixels[i] == null && target == null) {
    			count--;
    		} else if(pixels[i] != null && pixels[i].equals(target)) {
    			count--;
    		} 
    	}
		...
		for(int pixelsIndex = 0; pixelsIndex < pixels.length; pixelsIndex++) {
			if(pixels[pixelsIndex] == null && target == null) {
				continue;
			}else if(pixels[pixelsIndex] != null && pixels[pixelsIndex].equals(target)) {
    			continue;
    		} else {
    			colors[colorsIndex] = pixels[pixelsIndex];
				colorsIndex++;
    		}
    ...
    public static Rectangle findSmallest (Rectangle[] rectangles)
    {
    	...
    	int area = rectangles[0].height * rectangles[0].width;
    	for(int i =1; i < rectangles.length; i++) {
    		int tempArea = rectangles[i].height * rectangles[i].width;
    		if(tempArea <= area) {
     ...
     * 
     * largest value
     * make array size largest + 1
     * for loop(0...largest number
     * count int
     * for(data.length
     * if(i = data[j]
     * array = count then next number count = 0
     * 
     */
